...
int main(int argc, char *argv[]) {
    ...
    iQuotesHistory.get_candle(candle1, xtime::convert_cet_to_gmt(xtime::get_timestamp(1, 3, 2018, 12, 30, 0)));
    std::cout << "candle #1, open: " << candle1.open << " close: " << candle1.close << " date: " << xtime::get_str_date_time(candle1.timestamp) << std::endl;
    iQuotesHistory.get_candle(candle2, xtime::convert_cet_to_gmt(xtime::get_timestamp(1, 3, 2018, 12, 30, 0)));
    std::cout << "candle #2, open: " << candle2.open << " close: " << candle2.close << " date: " << xtime::get_str_date_time(candle2.timestamp) << std::endl;
    iQuotesHistory.get_candle(candle3, xtime::convert_cet_to_gmt(xtime::get_timestamp(1, 3, 2018, 12, 30, 0)));
    std::cout << "candle #2, open: " << candle3.open << " close: " << candle3.close << " date: " << xtime::get_str_date_time(candle3.timestamp) << std::endl;

    std::cout << "sequential read" << std::endl;
    // делаем последовательный запрос цены по дням
    for(int i = 1; i < 10; ++i) {
        xquotes_history::Candle candle;
        iQuotesHistory.get_candle(candle, xtime::convert_cet_to_gmt(xtime::get_timestamp(i, 3, 2018, 12, 30, 0)));
        std::cout << "candle #" << i << ", open: " << candle.open << " close: " << candle.close << " date: " << xtime::get_str_date_time(candle.timestamp) << std::endl;
    }
    // делаем последовательный запрос цены по минуте
    for(int i = 0; i < 10; ++i) {
        xquotes_history::Candle candle;
        iQuotesHistory.get_candle(candle, xtime::convert_cet_to_gmt(xtime::get_timestamp(1, 3, 2018, 12, 0 + i, 0)));
        std::cout << "candle #" << i << ", open: " << candle.open << " close: " << candle.close << " date: " << xtime::get_str_date_time(candle.timestamp) << std::endl;
    }
    // делаем запрос цены из далекого прошлого
    std::cout << "check the time limit" << std::endl;
    iQuotesHistory.get_candle(candle1, xtime::convert_cet_to_gmt(xtime::get_timestamp(1, 3, 1980, 12, 30, 0)));
    std::cout << "candle past, open: " << candle1.open << " close: " << candle1.close << " date: " << xtime::get_str_date_time(candle1.timestamp) << std::endl;
    iQuotesHistory.get_candle(candle2, xtime::convert_cet_to_gmt(xtime::get_timestamp(1, 3, 4000, 12, 30, 0)));
    std::cout << "candle future, open: " << candle2.open << " close: " << candle2.close << " date: " << xtime::get_str_date_time(candle2.timestamp) << std::endl;
    ...
    std::cout << "start timestamp: " << xtime::get_str_date_time(xtime::get_timestamp(1, 3, 2018)) << std::endl;
